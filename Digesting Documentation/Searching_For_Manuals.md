...
## introduction
This Problem was a test of one's attention to detail and built upon the last problem, although the approach was slightly more unorthodox
## approach
- I start by using `man man` as instructed in the problem
- After a very long time of going through the whole documentation, I stumble across `man -K` which allows me to search for specific strings
- knowing this I use `man -K /challenge/challenge` to search for manuals that reference this string, finally finding appropriate documentation
- I find the `caznxb` argument , I follow it with `/challenge/challenge --caznxb 040 ` as 040 was the key that was specified in the documentation
## code
```bash
...
hacker@man~searching-for-manuals:~$ /challenge/challenge --caznxb 040
Correct usage! Your flag: pwn.college{caIzVGEnx0b40Wk6xV3uPezse-E.dZTM4QDLzgTN0czW}
```
